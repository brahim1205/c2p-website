...
import { deleteProjectCenterProjection, persistProjectCenterProjection } from './platform-project-center-projection.js';
...
export class PlatformPersistenceService {
  ...
  private async persistNormalizedProjection(tx: Prisma.TransactionClient, rowsByTable: Record<string, Row[]>) {
    ...
    await persistProjectCenterProjection(tx, { projects: rowsByTable.projects ?? [], project_milestones: rowsByTable.project_milestones ?? [], project_documents: rowsByTable.project_documents ?? [], project_history: rowsByTable.project_history ?? [], project_funding_rounds: rowsByTable.project_funding_rounds ?? [], funding_investors: rowsByTable.funding_investors ?? [], project_partnerships: rowsByTable.project_partnerships ?? [], project_tracking: rowsByTable.project_tracking ?? [], project_collaborations: rowsByTable.project_collaborations ?? [] });
  }
  ...
  private async deleteNormalizedProjection(tx: Prisma.TransactionClient, removalsByTable: Record<string, string[]>) {
    ...
    await deleteProjectCenterProjection(tx, { projects: removalsByTable.projects ?? [], project_milestones: removalsByTable.project_milestones ?? [], project_documents: removalsByTable.project_documents ?? [], project_history: removalsByTable.project_history ?? [], project_funding_rounds: removalsByTable.project_funding_rounds ?? [], funding_investors: removalsByTable.funding_investors ?? [], project_partnerships: removalsByTable.project_partnerships ?? [], project_tracking: removalsByTable.project_tracking ?? [], project_collaborations: removalsByTable.project_collaborations ?? [] });
  }
