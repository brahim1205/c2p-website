...
export class AuthService {
  ...
  private getOAuthCallbackUrl(provider: OAuthProvider, request: AuthenticatedRequest) {
  ...
  private getFrontendUrl(path: string, request: AuthenticatedRequest) {
  ...
  private normalizeOAuthRole(role?: Role): Role {
  ...
  private decodeOAuthState(value?: string): OAuthStatePayload {
    const secret = this.config.oauthStateSecret;
    ...
    return {
      role: this.normalizeOAuthRole(payload.role),
  ...
  async startOAuth(
    ...
    const provider = this.normalizeOAuthProvider(rawProvider);
    const providerConfig = this.getOAuthConfig(provider);
    const callbackUrl = this.getOAuthCallbackUrl(provider, request);
  ...
  private async exchangeOAuthCode(provider: OAuthProvider, code: string, callbackUrl: string) {
    const providerConfig = this.getOAuthConfig(provider);
  ...
  private async fetchOAuthProfile(provider: OAuthProvider, accessToken: string): Promise<OAuthProviderProfile> {
    const providerConfig = this.getOAuthConfig(provider);
  ...
  private userNeedsProfileOnboarding(user: StoredUser) {
  ...
  async completeOAuth(
    ...
    return this.runSerializedMutation(async () => {
      ...
      const callbackUrl = this.getOAuthCallbackUrl(provider, request);
      const token = await this.exchangeOAuthCode(provider, payload.code, callbackUrl);
      ...
      const targetPath = this.userNeedsProfileOnboarding(user)
        ? `/auth/onboarding/profil?next=${encodeURIComponent(state.returnTo)}`
        : state.returnTo;
      return response.redirect(this.getFrontendUrl(targetPath, request));
    });
  }
