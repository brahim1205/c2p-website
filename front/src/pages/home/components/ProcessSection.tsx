    ...
    >
      ...
      {index < steps.length - 1 && (
        <div className="hidden lg:block absolute top-32 left-[calc(100%-16px)] w-[calc(100%-32px)] h-[2px] z-0">
          <div className={`h-full bg-[#5fa6f3]/20 rounded-full transition-all duration-1000 delay-[${index * 200 + 500}ms] ${isVisible ? 'w-full' : 'w-0'}`}></div>
        </div>
      )}
      ...
      <div className="relative bg-white rounded-[20px] overflow-hidden hover:shadow-2xl hover:-translate-y-3 transition-all duration-500 z-10">
        <div className={`${step.bg} h-[240px] lg:h-[280px] flex items-center justify-center p-6 lg:p-8 relative overflow-hidden`}>
          ...
          {/* Floating icon */}
          <div
            className={`absolute top-4 left-4 w-12 h-12 bg-[#5fa6f3] rounded-xl flex items-center justify-center shadow-lg transition-all duration-500 ${
        ...
        <div className="p-6">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-10 h-10 flex items-center justify-center bg-[#5fa6f3] text-white rounded-full text-sm font-bold shadow-md">
              {index + 1}
            </div>
            <div className="text-[#06053a] font-bold text-lg">{step.title}</div>
          </div>
...
export default function ProcessSection() {
  ...
  return (
    <section ref={sectionRef} className="py-24 lg:py-32 px-4 sm:px-6 lg:px-20 bg-[#ffffff] relative overflow-hidden">
      {/* Decorative elements */}
      <div className="absolute top-1/3 -left-32 w-[400px] h-[400px] bg-[#5fa6f3]/5 rounded-full blur-[100px] pointer-events-none"></div>
      <div className="absolute bottom-1/4 -right-32 w-[300px] h-[300px] bg-[#06053a]/5 rounded-full blur-[80px] pointer-events-none"></div>
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className={`mb-10 sm:mb-16 lg:mb-20 transition-all duration-1000 ease-out ${sectionVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <div className="text-center">
            <div className="inline-flex items-center gap-2 mb-5">
              ...
              <span className="text-[#06053a] text-sm font-medium uppercase tracking-wider">Comment ça marche</span>
            </div>
            <h2 className="text-[#06053a] text-2xl sm:text-3xl lg:text-[48px] leading-tight mb-3 sm:mb-4">
              <span className="font-bold">4 Étapes Pour</span><br />
              <span className="font-serif italic text-[#5fa6f3]">Votre Réussite</span>
            </h2>
        ...
        <div className={`mt-16 text-center transition-all duration-1000 delay-700 ${sectionVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <a
            href="/auth/register"
            className="group inline-flex items-center gap-3 bg-[#06053a] text-white px-8 py-4 rounded-full text-sm font-semibold hover:bg-[#5fa6f3] hover:scale-105 active:scale-95 transition-all duration-300 whitespace-nowrap shadow-lg"
          >
