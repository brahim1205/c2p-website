...
export default function PartnersSection() {
  ...
  return (
    <section ref={sectionRef} className="py-20 lg:py-28 bg-[#faf8f3] overflow-hidden relative">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-20 mb-12">
        <div className="text-center">
          <div className="inline-flex items-center gap-2 mb-5">
            ...
            <span className="text-[#1a2b4a] text-sm font-medium uppercase tracking-wider">Nos partenaires</span>
          </div>
          <h2 className="text-[#1a2b4a] font-bold text-2xl sm:text-3xl lg:text-[48px] leading-tight mb-4">
            Ils nous font <span className="text-[#14B8A6]">confiance</span>
          </h2>
      ...
      >
        {duplicated.map((partner, i) => (
          <div
            key={`${partner.name}-${i}`}
            className={`flex-shrink-0 w-[180px] h-[100px] bg-white rounded-xl border border-gray-100 flex flex-col items-center justify-center gap-2 hover:border-[#14B8A6]/30 hover:shadow-md transition-all duration-300 cursor-default ${
          ...
          >
            <div className="w-10 h-10 rounded-lg bg-[#14B8A6]/10 flex items-center justify-center">
              <div className="w-6 h-6 flex items-center justify-center">
                <i className={`${partner.icon} text-[#14B8A6] text-xl`}></i>
              </div>
            </div>
            <span className="text-[#1a2b4a] font-semibold text-sm">{partner.name}</span>
          </div>
        ))}
      </div>
    </section>
