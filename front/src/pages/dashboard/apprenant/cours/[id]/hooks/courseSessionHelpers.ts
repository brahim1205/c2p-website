...
import type { Course, EntityId, Lesson } from '../types';

export function getCourseLessons(course: Course) {
  return course.modules.flatMap((module) => module.lessons);
}

export function findResumeLesson(course: Course, completedLessons: Set<EntityId>) {
...
export function getInitialVideoTime(course: Course | null, lessonId: EntityId) {
  if (!course) return 0;
...
export function getCourseProgress(course: Course | null, completedLessons: Set<EntityId>) {
