...
function StatItem({ end, suffix, label, icon, delay }: {
  ...
  return (
    <div ref={ref} className="text-center group">
      <div className="w-14 h-14 md:w-16 md:h-16 rounded-2xl bg-[#14B8A6]/10 flex items-center justify-center mx-auto mb-4 group-hover:bg-[#14B8A6] group-hover:scale-110 transition-all duration-300">
        <div className="w-7 h-7 md:w-8 md:h-8 flex items-center justify-center">
          <i className={`${icon} text-xl md:text-2xl text-[#14B8A6] group-hover:text-white transition-colors`}></i>
        </div>
      </div>
      <div className="text-[#1a2b4a] font-bold text-3xl md:text-4xl lg:text-5xl transition-transform duration-300 group-hover:scale-105">
...
export default function Stats() {
  ...
  return (
    <section ref={ref} className="py-16 lg:py-20 px-4 sm:px-6 lg:px-20 bg-[#1a2b4a] relative overflow-hidden">
      <div className="absolute top-0 left-1/4 w-[400px] h-[400px] bg-[#14B8A6]/10 rounded-full blur-[120px] pointer-events-none"></div>
      <div className="absolute bottom-0 right-1/4 w-[300px] h-[300px] bg-[#14B8A6]/5 rounded-full blur-[100px] pointer-events-none"></div>
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <div className={`text-center mb-12 lg:mb-14 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <h2 className="text-white font-bold text-2xl sm:text-3xl lg:text-4xl mb-3">
            C2P en <span className="text-[#14B8A6]">Chiffres</span>
          </h2>
