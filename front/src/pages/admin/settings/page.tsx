    ...
    <AdminLayout>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 mb-6">
          <div className="flex border-b border-gray-200 overflow-x-auto" role="tablist" aria-label="Sections du parametrage plateforme">
            {[
              ...
              <button key={tab.id} type="button" role="tab" aria-selected={activeTab === tab.id} aria-controls={`admin-settings-panel-${tab.id}`} id={`admin-settings-tab-${tab.id}`} onClick={() => setActiveTab(tab.id as typeof activeTab)} className={`flex items-center gap-2 px-4 lg:px-6 py-4 font-medium whitespace-nowrap transition-colors text-sm ${activeTab === tab.id ? 'text-teal-600 border-b-2 border-teal-600' : 'text-gray-600 hover:text-gray-900'}`}>
        ...
        {activeTab === 'categories' && (
          <div className="space-y-6" role="tabpanel" id="admin-settings-panel-categories" aria-labelledby="admin-settings-tab-categories">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <button type="button" onClick={() => setShowAddCategory(true)} aria-label="Ajouter une categorie" className="px-4 py-2 bg-teal-600 text-white rounded-lg text-sm font-medium hover:bg-teal-700 transition-colors whitespace-nowrap flex items-center gap-2">
                ...
                return (
                  <div key={category.id} className={`bg-white rounded-xl border p-4 transition-all hover:shadow-md ${category.active ? 'border-gray-200' : 'border-gray-200 opacity-60'}`}>
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <button type="button" onClick={() => void handleToggleCategory(category.id)} aria-pressed={category.active} aria-label={`${category.active ? 'Desactiver' : 'Activer'} la categorie ${category.name}`} className="relative inline-flex items-center cursor-pointer">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button type="button" onClick={() => void handleDeleteCategory(category.id)} aria-label={`Supprimer la categorie ${category.name}`} className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-red-50 transition-colors" title="Supprimer">
                  ...
                  <div key={rule.id} className={`flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-lg border transition-colors ${editingRule === rule.id ? 'border-teal-300 bg-teal-50/30' : 'border-gray-100 hover:border-gray-200'}`}>
                    ...
                    <div className="flex items-center gap-3">
                      {editingRule === rule.id ? (
                        <>
                          {rule.type === 'toggle' ? (
                            <input aria-label={`Activer ou desactiver ${rule.label}`} type="checkbox" checked={editValues[rule.id] !== undefined ? Boolean(editValues[rule.id]) : Boolean(rule.value)} onChange={(e) => setEditValues({ ...editValues, [rule.id]: e.target.checked })} />
                          ) : (
                            <input aria-label={`Valeur pour ${rule.label}`} type={rule.type === 'text' ? 'text' : 'number'} value={editValues[rule.id] !== undefined ? String(editValues[rule.id]) : String(rule.value)} onChange={(e) => setEditValues({ ...editValues, [rule.id]: e.target.value })} className="w-28 px-2 py-1.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 text-right" />
                          )}
                        </>
                      ) : (
                      ...
                      <button type="button" onClick={() => editingRule === rule.id ? void handleSaveRule(rule) : (setEditingRule(rule.id), setEditValues({ ...editValues, [rule.id]: rule.value }))} aria-label={editingRule === rule.id ? `Enregistrer la regle ${rule.label}` : `Modifier la regle ${rule.label}`} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-gray-100 transition-colors">
          ...
          <div className="space-y-6" role="tabpanel" id="admin-settings-panel-integrations" aria-labelledby="admin-settings-tab-integrations">
            {integrations.map((integration) => (
              <div key={integration.id} className="bg-white rounded-xl border border-gray-200 p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2">
                  {integration.status === 'connected' && <button type="button" onClick={() => void handleSyncIntegration(integration)} aria-label={`Synchroniser l integration ${integration.name}`} className="px-4 py-2 border border-gray-200 text-gray-700 rounded-lg text-xs font-medium hover:bg-gray-50 transition-colors whitespace-nowrap">Synchroniser</button>}
                  <button type="button" onClick={() => void handleToggleIntegration(integration)} aria-pressed={integration.status === 'connected'} aria-label={`${integration.status === 'connected' ? 'Deconnecter' : 'Connecter'} l integration ${integration.name}`} className={`px-4 py-2 rounded-lg text-xs font-medium transition-colors whitespace-nowrap ${integration.status === 'connected' ? 'border border-red-200 text-red-600 hover:bg-red-50' : 'bg-teal-600 text-white hover:bg-teal-700'}`}>
        ...
        {showAddCategory && (
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl max-w-md w-full p-6" role="dialog" aria-modal="true" aria-labelledby="admin-settings-add-category-title">
              <div className="flex items-center justify-between mb-6">
                <h3 id="admin-settings-add-category-title" className="text-lg font-bold text-gray-900">Nouvelle categorie</h3>
                <button type="button" onClick={() => setShowAddCategory(false)} aria-label="Fermer la creation de categorie" className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-gray-100 transition-colors"><i className="ri-close-line text-gray-500 text-xl"></i></button>
              </div>
              <div className="space-y-4">
                <div>
                  <label htmlFor="admin-category-name" className="mb-1 block text-sm font-medium text-gray-700">Nom de la categorie</label>
                  <input id="admin-category-name" type="text" value={newCategory.name} onChange={(e) => setNewCategory({ ...newCategory, name: e.target.value })} placeholder="Ex: Menuiserie" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-teal-500" />
                </div>
                <div>
                  <label htmlFor="admin-category-type" className="mb-1 block text-sm font-medium text-gray-700">Type de categorie</label>
                  <select id="admin-category-type" value={newCategory.type} onChange={(e) => setNewCategory({ ...newCategory, type: e.target.value as AdminCategory['type'] })} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-teal-500">
              ...
              <div className="flex gap-3 mt-6">
                <button type="button" onClick={() => setShowAddCategory(false)} className="flex-1 px-4 py-2.5 border border-gray-200 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-50 transition-colors">Annuler</button>
                <button type="button" onClick={() => void handleAddCategory()} disabled={!newCategory.name.trim()} className={`flex-1 px-4 py-2.5 rounded-lg text-sm font-medium transition-colors ${newCategory.name.trim() ? 'bg-teal-600 text-white hover:bg-teal-700' : 'bg-gray-300 text-gray-500 cursor-not-allowed'}`}>Creer</button>
              </div>
            </div>
          </div>
        )}
